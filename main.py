...
from report import generate_daily_report  # Импорт функции из report.py
...
def send_email(to_mail):
    ...
    msg["From"] = from_mail
    msg["To"] = to_mail
    msg["Subject"] = Header('Ежедневный отчет', 'utf-8')
    msg["Date"] = formatdate(localtime=True)
    msg.attach(MIMEText("Добрый день! Прикрепляем ежедневный отчет.", 'plain', 'utf-8'))

    # Логируем создание сообщения
    logging.info('Создано сообщение с темой: %s для %s', msg["Subject"], to_mail)

    # Генерируем и прикрепляем ежедневный отчет
    generate_daily_report(attachment_path)
